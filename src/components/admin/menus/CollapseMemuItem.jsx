...
const CollapseMemuItem = props => {
  ...
  return (
    <>
      <Tooltip color="orange" placement="right" title={text} open={isCloseMenu ? undefined : false}>
        <Link href={pushTo ? pushTo : ""}>
          <div
            ...
            {!isCloseMenu && (
              <RightOutlined className={`icon-right-menu ${isActive && listSubMenu?.length > 0 && "rotate-90"}`} />
            )}
          </div>
        </Link>
      </Tooltip>
